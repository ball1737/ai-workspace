...
| happywork-sale-cms/src/sections/package-config-feature/package-config-feature-view.tsx | 2026-05-07 | yes | Wave 6.1 — full rewrite to drop USE_LEGACY_CONFIG_FEATURE branch + legacy fetch/toggle handlers; keep only CompanyFeaturesTabView render path + EditPackageDialog; package-info read still wired through saleDashboardPackageConfigFeature slice (kept for shared use) |
| happywork-sale-cms/src/store/actions/sale-dashboard-package-config-feature.ts | 2026-05-07 | yes | Wave 6.1 — removed 6 action creators (sdGetFeatures{Request,Success,Failure} + sdToggleFeature{Request,Success,Failure}); kept package-info / package-options / preview / change-package triplets |
| happywork-sale-cms/src/store/reducers/sale-dashboard-package-config-feature.ts | 2026-05-07 | yes | Wave 6.1 — removed 6 handlers (features GET + feature toggle) + initial state fields `features`/`isToggling` |
| happywork-sale-cms/src/store/sagas/sale-dashboard-package-config-feature.ts | 2026-05-07 | yes | Wave 6.1 — removed `getFeaturesSaga` + `toggleFeatureSaga` + 2 takeLatest watcher entries; remaining sagas (getPackageInfo / getPackageOptions / preview / change) untouched |
| happywork-sale-cms/src/store/services/sale-dashboard.ts | 2026-05-07 | yes | Wave 6.1 — removed URL builders `featureManagement(companyUuid)` + `featureManagementToggle(companyUuid)` |
| happywork-sale-cms/src/types/store/sale-dashboard-package-config-feature.ts | 2026-05-07 | yes | Wave 6.1 — removed `features: FeatureItem[]` + `isToggling: boolean` from Root; removed `FeatureItem` + `ToggleFeatureRequest` types |
| happywork-sale-cms/src/store/actions/sale-dashboard-feature-management.ts | 2026-05-07 | yes | Wave 6.1 — header comment updated to note Wave 6.1 removal of legacy actions (functional code unchanged) |
| happywork-sale-cms/src/store/actions/sale-dashboard-company-features.ts | 2026-05-07 | yes | Wave 6.1 — header comment updated to note Wave 6.1 removal of legacy actions (functional code unchanged) |
| happywork-backend/src/api/v2/sale-dashboard/sale-dashboard.routes.ts | 2026-05-07 | yes | Wave 6.1 — removed `featureManagementRouter` import + `/feature-management` mount line |
| happywork-backend/src/api/v2/sale-dashboard/featureManagement/{controller,routes}.ts | 2026-05-07 | deleted | Wave 6.1 — entire directory removed (no real caller; FE flag was hard-off since Phase 3) |
| happywork-backend/src/modules/v1/featureManagement/{interface,repository,service}.ts | 2026-05-07 | deleted | Wave 6.1 — entire directory removed |
| happywork-backend/src/constant/featureDefinitions.ts | 2026-05-07 | deleted | Wave 6.1 — only consumer was deleted v1/featureManagement.service.ts |
| happywork-backend/src/database/postgresql/models/data/saleDashboardDisabledFeature.model.ts | 2026-05-07 | deleted | Wave 6.1 — only consumer was deleted v1/featureManagement.repository.ts |
| happywork-backend/src/database/postgresql/migrations/data/20260507-0900-drop-sale-dashboard-disabled-features-table.ts | 2026-05-07 | yes (created) | Wave 6.1 — drop migration; up=dropTableIfExists, down=re-create original schema verbatim from 20260204-0704 (rollback ladder preserved); NOT yet run on dev DB (Lead will run after verify) |
| happywork-backend/tests/integration/sale-dashboard/feature-management.integration.spec.ts | 2026-05-07 | no | INSPECTED — file is misnamed; describe blocks cover Phase 2 Feature/Package/Addon Master CRUD (NOT legacy /feature-management/:companyUuid). Contains 20+ tests for current production endpoints. Decision: KEEP (deletion would lose live test coverage). Flagged to Lead for filename rename in a future cleanup. |
